...
public class OctoMemPipeline implements Pipeline {
            ...
            try {
                ...
                if (target instanceof SeqFileTarget) {
                    ...
                    if (target instanceof AvroFileTarget) {
                        ...
                        Path outputPath = new Path(path, "out.txt");
                        FSDataOutputStream os = fs.create(outputPath);
    ...
    @Override
    public <S> PCollection<S> union(List<PCollection<S>> pCollections) {
        List<S> output = Lists.newArrayList();
        for (PCollection<S> pcollect : pCollections) {
            Iterables.addAll(output, pcollect.materialize());
        }
        return new MemCollection<S>(output, pCollections.get(0).getPType());
    }
